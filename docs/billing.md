...
- **운영 진단**: 결제 대사를 실행할 때마다 `last_reconciled_at`과 `reconciliation_error`를 기록합니다. 결제 운영자는 `/support/payments/operator`에서 전체 사용자의 최근 결제, 대기·실패 상태, 환불, 구독 상태, Toss 불일치를 확인하고 수동 대사를 실행할 수 있습니다.
